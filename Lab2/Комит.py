...
Fuv.append(fuv(dispersion(y)[0], dispersion(y)[2]))
Fuv.append(fuv(dispersion(y)[1], dispersion(y)[2]))
if rows == 4:
    Fuv.append(fuv(dispersion(y)[0], dispersion(y)[3]))
    Fuv.append(fuv(dispersion(y)[1], dispersion(y)[3]))
    Fuv.append(fuv(dispersion(y)[2], dispersion(y)[3]))

for i in range(6):
    teta.append(((m - 2) / m) * Fuv[i])

for i in range(6):
    Ruv.append(abs(teta[i] - 1) / teta_sigma)
